...
class VideoPublishForm(forms.ModelForm):
    ...
    desc = forms.CharField(max_length=300, required=True,
                           error_messages={'max_length': '描述最多300个字', 'required': '描述不能为空'},
                           widget=forms.Textarea(attrs={'placeholder': '请输入视频描述'}))

    class Meta:
        model = Video
        fields = ['title', 'status', 'cover', 'classification', 'desc']
