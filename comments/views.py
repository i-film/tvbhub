...
def submit_comment(request, pk):
    ...
    if was_limited:
        return JsonResponse({'code': 1, 'msg': '评论太频繁啦，请1分钟后再试呐'})
        pass
    ...
    if form.is_valid():
        ...
        html = render_to_string('comments/comment_single.html', {'comments': comments})

        return JsonResponse({'code': 0, 'html': html})
    return JsonResponse({'code': 1, 'msg': 'Oops～ 评论失败咯!'})
...
def get_comments(request):
    ...
    if len(rows) > 0:
        code = 0
        html = render_to_string('comments/comment_single.html', {'comments': rows})
    else:
        code = 1
        html = ''
